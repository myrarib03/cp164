...
class Movie:
    ...
    def genres_menu():
        ...
        values = 0
        string = "Genres\n"
        first_line = len(Movie.GENRES)
        max_width = first_line-1

        for _ in range(max_width):
            values += 1
            align = len(str(values))

        while count < first_line:
            string += f"{str(count):>{align}} {Movie.GENRES[count]}\n"
            count += 1
        return string
...
Genres:   {}""".format(self.title, self.year, self.director, self.rating, genres_list)
    ...
    def genres_string(self):
        ...
        # self.genres = list of genres given (int) = [2,4]
        length = len(self.genres)
        strings = ""
        count = 0
        while count < length:
            genre = self.GENRES[self.genres[count]]
            strings += f"{genre}, "
            count += 1
        string = strings[:-2]
        return string
    ...
    def genres_list_string(self):
        ...
        while count < len(self.genres):
            string += f"{self.genres[count]},"
            count += 1
