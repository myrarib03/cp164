...
class List:
    ...
    def prepend(self, value):
        ...
        if self._front is None:
            element = _List_Node(value, None)
            self._rear = element
            self._front = element
        else:
            element = _List_Node(value, self._front)
            self._front = element
        self._count += 1
        return None
    ...
    def insert(self, i, value):
        ...
        if -1 * self._count >= i:
            ...
            if 0 < i:
                ...
                values = -1 * self._count
                front = self._front
                while values < i - 1:
                    front = front._next
                    values += 1
            nexts = front._next
            element = _List_Node(value, nexts)
            front._next = element
            self._count += 1
        return None
    ...
    def _linear_search(self, key):
        ...
        values = False
        index = 0
        while current is not None and not values:

            if key is current._value:
                values = True
            else:
                previous = current
    ...
    def max(self):
        ...
        front = self._front
        maximum = front._value

        while front is not None:
            if front._value > maximum:
                maximum = front._value
            front = front._next

        max_data = deepcopy(maximum)
    ...
    def min(self):
        ...
        front = self._front
        minimum = front._value
        while front is not None:
            if minimum > front._value:
                minimum = front._value
            front = front._next

        min_data = deepcopy(minimum)
        return min_data
